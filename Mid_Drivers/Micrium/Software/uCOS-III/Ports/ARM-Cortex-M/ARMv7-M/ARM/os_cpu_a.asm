...
;********************************************************************************************************
    ...
    IMPORT  OS_KA_BASEPRI_Boundary  ;IMPORT 声明标号来自外部文件
...
NVIC_SYSPRI14   EQU     0xE000ED22       ; 系统优先级寄存器                       ; System priority register (priority 14).
NVIC_PENDSV_PRI EQU           0xFF       ; PendSV中断和系统节拍中断                          ; PendSV priority value (lowest).
NVIC_PENDSVSET  EQU     0x10000000       ; 触发软件中断的值                       ; Value to trigger PendSV exception.


;    NVIC_INT_CTRL 为中断控制寄存器，地址为0xE000ED04，
;    NVIC_SYSPRI4 为PendSV中断优先级寄存器，地址为0xE000ED22，
;    NVIC_PENDSV_PRI为PendSV和Systick的中断优先级，这里为0xFFFF，都为最低优先级；
;    NVIC_PENDSVSET 可以触发软件中断，通过给中断控制寄存器NVIC_INT_CTRL 的bit28位写1来触发软件中断，因此NVIC_PENDSVSET为0x10000000。
...
OS_CPU_FP_Reg_Push
    MRS     R1, PSP      ;psp是进程堆栈指针  MRS:加载特殊功能寄存器的值到通用寄存器  ; PSP is process stack pointer
    CBZ     R1, OS_CPU_FP_nosave  ;CBZ:比较，如果结果为0就转移                       ; Skip FP register save the first time

    VSTMDB  R0!, {S16-S31}
    LDR     R1, =OSTCBCurPtr
    LDR     R2, [R1]   
    STR     R0, [R2]    ;STR:把一个寄存器按字存储到存储器中
OS_CPU_FP_nosave
    BX      LR
...
PendSV_Handler
    CPSID   I            ;关闭中断                                       ; Cortex-M7 errata notice. See Note #5
    ...
    MRS     R0, PSP              ;R0 = PSP                               ; PSP is process stack pointer
