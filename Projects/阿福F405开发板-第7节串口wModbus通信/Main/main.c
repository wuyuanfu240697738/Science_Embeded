...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //初使化串口
    wSerial_Init(0,wSerial_Pin_0,wSerial_BaudRate,wSerial_StopBits,wSerial_Parity);

	MB_Init( &mt,MB_MODE_RTU,1,0,&mutex);//站号为1的从机 rtu模式
	
	while (1)
    {  
        //阻塞接收数据
        int Rlen = wSerial_Receive(0,data,500,0);
        
        if(Rlen > 0)
        {//接收到数据
			
			int Slen = sMB_Deal(&mt,data,Rlen);//从机处理
			if(Slen > 0)
			{
				wSerial_Transmit(0,data,Slen,2000);
			}
        } 
    }	
}
